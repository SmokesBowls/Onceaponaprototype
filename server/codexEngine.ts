...
/**
 * Stop-words and non-entity nouns to avoid turning every trivial word into an entity
 */
const TRIVIAL_NOUNS = new Set([
  'the', 'a', 'an', 'this', 'that', 'it', 'something', 'anything', 'nothing',
  'time', 'way', 'day', 'moment', 'shadow', 'silence', 'air', 'ground', 'side',
  'step', 'hand', 'eye', 'head', 'face', 'voice', 'sound', 'breath', 'distance',
]);
    ...
    if (glowMatch) {
      ...
      if (priorGlowClaim) {
        ...
        if (priorColor && priorColor !== newColor) {
          ...
          // Mark prior claim as contradicted and record both without overwriting
...
export function synthesizeCodex(project: StoryProject): CodexEntity[] {
  ...
  // Helper to ensure entity exists
  const getOrCreate = (
    id: string,
    workingLabel: string,
    defaultType: string,
  ...
  for (const beat of project.manuscript) {
    ...
    for (const [entId, ent] of Array.from(codexMap.entries())) {
      ...
      if (mentionedInBeat) {
        ...
        // Detect interactions (possession vs seeing vs resting)
        ...
        // Update holder ONLY on explicit possession
  ...
  for (const [entId, ent] of Array.from(codexMap.entries())) {
    ...
    // Check for contradiction in claims
    ...
    if (ent.is_author_locked) {
      ...
      // Contradictory evidence must NOT increase reliability as though it were corroborated
      if (hasContradiction && calculated > 0.45) {
        calculated = 0.45; // Cap reliability while active contradiction remains unresolved
      }
      ent.reliability = calculated;
    }

    // Update classification confidence based on evidence count
